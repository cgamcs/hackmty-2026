...
begin
  ...
  -- The audited tables do not share the same primary-key column: invoices
  -- uses uuid, obligations/suppressions use id and terms has a composite key.
  -- Reading the trigger record as JSON avoids referencing a field that does
  -- not exist on a particular table (which would abort the original write).
  tid := nullif(row_data ->> 'tenant_id', '')::uuid;
  key := coalesce(
    row_data ->> 'uuid',
    row_data ->> 'id',
    concat_ws(':', row_data ->> 'tenant_id', row_data ->> 'counterparty_rfc'),
    ''
  );
