...
def _set_cookie(response: Response, token: str) -> None:
    response.set_cookie(
        ...
        samesite="lax",
        secure=not DEMO_MODE,    # plain http only while demoing locally
...
def register(response: Response, payload: dict):
    ...
    with connect() as conn:
        # register_tenant is SECURITY DEFINER: RLS on `tenants` cannot be satisfied before
        # the tenant exists, and INSERT ... RETURNING must also pass the SELECT policy.
        # See db/tiger/005_register.sql for why this is the only privileged entry point.
        try:
            created = fetch_one(
